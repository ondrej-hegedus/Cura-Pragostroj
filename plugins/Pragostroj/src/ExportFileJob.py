...
from Pragostroj.src.MeshFormatHandler import MeshFormatHandler
...
class ExportFileJob(WriteFileJob):
    ...
    def __init__(self, file_handler: Optional[FileHandler], nodes: List[SceneNode], firmware_version: str) -> None:
        ...
        if not self._mesh_format_handler.is_valid:
            ...
            return
        print('----------ExportFileJob------- queriyng gcode_dict')
        processed_nodes = Application.getInstance().getController().getScene().gcode_dict
        ...
        super().__init__(self._mesh_format_handler.writer, self._mesh_format_handler.createStream(), processed_nodes,
                         self._mesh_format_handler.file_mode)
